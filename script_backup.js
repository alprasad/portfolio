...
document.addEventListener('DOMContentLoaded', () => {
    ...
    searchInput.addEventListener('click', () => renderDropdown(searchInput.value));
    ...
    searchInput.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter') {
            ...
            const item = SEARCH_ITEMS.filter(it => {
                const q = searchInput.value.trim().toLowerCase();
                return q ? (it.label.toLowerCase().includes(q) || it.sub.toLowerCase().includes(q)) : true;
            })[idx];
    ...
    document.addEventListener('mousedown', (e) => {
        if (!e.target.closest('#hdrSearchWrap')) closeDropdown();
    });
    ...
    // Event Delegation for Industry Experience Card Clicks
    document.addEventListener('click', (e) => {
    ...
    // Support Keyboard Enter/Space on cards
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            const card = e.target.closest('.ent-card');
            if (card) {
